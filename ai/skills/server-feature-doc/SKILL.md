...
description: standard protocol for creating and maintaining backend NestJS feature-specific Agent.md files. Requires documenting core workflows using Mermaid flowcharts.
---

# Skill: Server Feature Document Protocol (后端模块说明规范)

## 核心原则 / Core Principle
本规范旨在指导 AI 智能体与开发者为**后端（NestJS）**的每个核心功能模块撰写统一、精准的 `Agent.md` 指南。修改该模块的代码或评估功能时，AI 能够通过此指南瞬间熟悉后端架构与逻辑。**本规范强制要求包含描述数据流/控制流的 Mermaid 流程图**。
...
此处必须提供描述业务执行逻辑或数据流向的 Mermaid 流程图。例如：

```mermaid
graph TD
    Client[客户端请求] --> Guard[JWT/权限验证守卫]
    Guard --> Controller[控制器参数校验]
    Controller --> Service[业务逻辑服务]
    Service --> DB[(MongoDB 数据库)]
    Service --> ThirdParty[外部第三方服务]
```
...
1. **强制画图**：每个后端模块 `Agent.md` **必须**包含至少一个清晰的 Mermaid 流程图，准确反映代码的时序或流转。
2. **真实代码还原**：编写 `Agent.md` 时，模块内的类名、方法名和调用顺序必须与实际代码**完全一致**。
3. **渐进式维护**：修改后端控制器、服务或数据模型时，必须主动同步更新本模块的 `Agent.md`。
