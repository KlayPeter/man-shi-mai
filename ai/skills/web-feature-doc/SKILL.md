...
description: standard protocol for creating and maintaining frontend Next.js feature-specific Agent.md files. Use this to understand frontend components, layouts, client/server routing, state management (Zustand), and styling.
---

# Skill: Web Feature Document Protocol (前端模块说明规范)

## 核心原则 / Core Principle
本规范旨在指导 AI 智能体与开发者为**前端（Next.js App Router）**的每个核心页面与模块撰写统一、精准的 `Agent.md` 指南。使得每次修改该页面的逻辑或优化 UI 时，AI 能够快速还原组件层次、页面跳转以及状态管理流向。
...
- **页面功能**: 简述本页面提供给用户什么功能（如：展示面试历史记录、查看面试报告与评分、扫码登录）。
- **用户交互流程**: 描述用户在此页面上的典型交互动作（如：点击“开始面试” -> 选择简历 -> 跳转至面试室）。
...
1. **真实架构还原**：编写前端 `Agent.md` 时，必须结合 `src/` 的真实文件命名与结构，如实还原组件引用关系。
2. **防重构原则**：在修改 UI 代码时，严格参考 `Agent.md` 中的设计，杜绝在不了解组件依赖的情况下修改全局样式或 Zustand Actions。
