...
- `requestId` 现为必填；上线需先发布新版前端，再发布后端并提示旧标签页刷新。真实模型输出质量、OSS 文件解析与生产环境仍未验收。第一阶段仍未整体完成；代码已推送，尚未部署。

## 发布前边界审计

```mermaid
flowchart LR
    A[代码检查与构建通过] --> B[专用 Mongo + HTTP/SSE + 浏览器通过]
    B --> C[生产模式合成配置启动通过]
    C --> D[真实模型 / 百度识别 / OSS 待验收]
    D --> E[配额来源与正式发布决策]
```

- 未知后端异常不再向浏览器或日志输出原始错误消息与堆栈；请求日志只记录路径，客户端 trace ID 有长度和字符限制。原实现可能泄露连接串、供应商响应或简历查询文本。
- 生产模式默认不启用 CORS 或 Swagger；需要浏览器直连时显式配置 `CORS_ORIGINS`。示例 DeepSeek 密钥在生产配置中被拒绝。删除启动时输出完整数据库连接配置的日志。
- 使用专用 MongoDB 和合成配置启动完整生产模式 AppModule，根健康响应 200、`/api` 为 404、陌生 Origin 无 CORS 授权、启动日志未出现测试连接串或合成密钥。没有调用真实模型、语音、OSS 或支付服务。
- DeepSeek `/models` 只读核对显示现有开发密钥可用，模型列表包含 `deepseek-flash`，不包含本地原配置的 `deepseek-v4.1-flash`。已统一模型默认值、示例配置与本地开发配置，并修正 `MAX_TOKENS` 未被模型工厂读取的问题；仍需用受控预算验证真实生成质量。
- 邮箱注册的新账户一次性获得简历押题、专项面试、行为 / HR 面试各 1 次；登录不再次发放，既有账户不回填。正式支付仍关闭，小麦币仍为 0。上一版代码的 GitHub Actions CI 已通过；本次只做本地代码与 Mock 验证，按用户要求不做真实服务联调或生产发布。真实服务质量、生产 HTTPS/域名、可写日志目录或外部日志采集、部署顺序与回滚仍未验收。
