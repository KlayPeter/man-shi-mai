...
- 上一版代码的 GitHub Actions CI 已通过，代码已推送至 `main`，尚未部署。尚待确定新用户权益来源：当前注册后练习次数和小麦币均为 0，正式支付也关闭；若不设体验额度或人工发放，新用户无法进入核心练习。真实服务质量、生产 HTTPS/域名、可写日志目录或外部日志采集、部署顺序与回滚仍需验收。
