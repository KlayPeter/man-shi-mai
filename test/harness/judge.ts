...
const JudgeOutputSchema = z.object({
  metrics: z.object({
    technical_depth: z.number().describe('打分1-10。技术追问是否击中要害，是否有效探清了求职者的真实底座水平。'),
    question_relevance: z.number().describe('打分1-10。面试官提出的问题是否与职位描述（JD）、简历以及求职者上一步的回答强相关，有无走偏。'),
    tone: z.number().describe('打分1-10。语气是否符合专业面试官的素养，是否保持尊重，有无粗鲁用语。'),
    flow_control: z.number().describe('打分1-10。面试阶段流转是否顺畅，是否卡死在一个问题无限循环。'),
    hallucination_rate: z.number().describe('评分1-10。面试官提问中出现常识性技术概念混淆或自己脑补幻想的频率。分值越低说明越无幻觉，0分为完美。')
  }),
  justification: z.string().describe('详细的扣分和加分理由，列举具体话术对话进行举证。'),
  suggestions: z.array(z.string()).describe('优化面试官 Prompt 或流程控制的改进具体建议')
});
...
export async function runJudge(
  ...
  const transcriptText = transcripts
    .map((t, i) => `${i + 1}. [${t.role === 'interviewer' ? 'AI 面试官' : '候选人'}]: ${t.content}`)
    .join('\n\n');
