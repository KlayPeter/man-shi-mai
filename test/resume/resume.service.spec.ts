...
describe('ResumeService', () => {
  ...
  describe('getResumeDetail', () => {
    it('should query resume by id and user id', async () => {
      ...
      expect(model.findOne).toHaveBeenCalledWith({ _id: 'res-1', userId: 'user-123' });
  ...
  describe('updateResumeContent', () => {
    it('should update editorData and snapshot', async () => {
      ...
      const result = await service.updateResumeContent('user-123', 'res-1', dto);
      expect(model.findOneAndUpdate).toHaveBeenCalledWith(
        { _id: 'res-1', userId: 'user-123' },
        { editorData: dto.editorData, plainTextSnapshot: dto.plainTextSnapshot, status: 'ready' },
        { new: true }
      );
      expect(result._id).toBe('res-1');
    });
