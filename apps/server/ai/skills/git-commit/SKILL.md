...
- **`<scope>` (作用域)**:
  - 简述受影响的模块/文件夹，例如：`server` (后端服务), `interview` (面试主线), `db` (数据库/Schema), `ai` (大模型交互) 等。

- **`<subject>` (主题/描述)**:
  - 简短描述本次提交的主要改动。
  - **完全支持并推荐使用中文描述**。
  - **正确格式示例**：
    * `feat(server): 新增面试记录归档接口`
    * `fix(interview): 修复评估判断器 Zod 格式转换解析错误`
    * `refactor(db): 优化用户模型与索引定义`
    * `docs: 更新后端服务配置与环境变量说明`
  - **错误格式防范**：
