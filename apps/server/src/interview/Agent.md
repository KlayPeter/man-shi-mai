...
- 简历押题由 `InterviewQuizService` 处理，同一 UUID 请求 ID 和输入快照只扣一次；缓存命中和新结果统一由流式入口发送 `yati-complete` 并结束。失败事件带 `terminal` 标记；非终态错误应重试原请求，终态失败已核对权益，可发起新请求。
- 小麦币兑换要求 UUID 请求 ID，`QuotaLedgerService` 在账户单文档内同时扣币、增加次数，重复请求只应用一次；交易记录用操作 ID 幂等补写。浏览器丢失响应后应重试同一 ID。旧前端缺少请求 ID 会收到 400，需要配套发布；旧兑换流水不回填。

押题顺序为「创建 PENDING 消费记录 → 领取租约 → 读取简历 → 账本扣次 → 模型生成并校验 → 将完整输出和 SUCCESS 原子写在同一记录 → 补齐押题结果投影」。失败只在尚未尝试保存成功结果时走账本反向操作，退款也可按同一请求恢复；结果投影写入失败只补投影，不退款或重跑模型。旧版成功记录按原 resultId 重放，不修改或补造历史评分。取消 SSE 订阅不保证取消付费模型；租约过期重试可能再次调用模型，但账户只扣一次，旧任务不能覆盖新结果。真实模型输出质量另行验收。
