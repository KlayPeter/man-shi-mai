...
const blufferProfile: CandidateProfile = JSON.parse(
  fs.readFileSync(
    path.join(__dirname, 'candidate-profiles/bluffer.json'),
    'utf8',
  ),
);
...
const silentExpertProfile: CandidateProfile = JSON.parse(
  fs.readFileSync(
    path.join(__dirname, 'candidate-profiles/silent-expert.json'),
    'utf8',
  ),
);
...
async function runMockInterviewSession(profile: CandidateProfile) {
  ...
  const openingGenerator =
    agentService.generateInterviewQuestionStream(contextInput);
  ...
  if (finalOpeningOutput && finalOpeningOutput.metadata) {
    ...
    questionsAskedCount =
      finalOpeningOutput.metadata.questionsAskedCount ?? questionsAskedCount;
    extractedSkills =
      finalOpeningOutput.metadata.extractedSkills ?? extractedSkills;
  }
  ...
  for (let turn = 1; turn <= maxTurns; turn++) {
    ...
    const questionGenerator =
      agentService.generateInterviewQuestionStream(nextContext);
    ...
    if (agentOutput && agentOutput.metadata) {
      ...
      questionsAskedCount =
        agentOutput.metadata.questionsAskedCount ?? questionsAskedCount;
  ...
  console.log(
    `💾 原始对话记录已保存至: test/harness/logs/${transcriptFileName}`,
  );
  ...
  const averageScore = (
    (evalReport.metrics.technical_depth +
      ...
      evalReport.metrics.flow_control) /
    4
  ).toFixed(1);
