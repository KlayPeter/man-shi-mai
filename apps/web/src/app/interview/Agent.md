...
- `InterviewPreflight` 展示岗位、经历来源、三段流程、练习强度、回答方式和实际扣次规则。热身 / 标准 / 挑战分别为最多 15 / 30 / 45 分钟、6 / 12 / 18 次作答，服务端执行上限；试麦使用 InterviewRecorder，本机录音与回听，不调用 ASR 或上传 API，不建立面试场次。
...
- 强度选择存入 practiceIntensity；开场请求的 pendingStart 冻结该选择，断线重试不能悄悄改强度。进入面试后使用服务端确认的强度与时长显示，恢复时也从服务器重读；旧场次缺少强度时沿用原有时长，不改写历史数据。
...
- 简历押题沿用确认与 SSE 入口。请求前把 UUID 与岗位、简历输入快照保存在当前账户的 `active-interview`；网络错误、刷新和非终态服务错误使用原请求重放。仅收到带有效 resultId 的 `yati-complete` 才清除待确认状态；`complete` 只关闭流，不代表业务成功。服务端终态失败会清除待确认请求，可重新开始；旧本地记录缺少完整快照时不自动生成新扣次请求。
...
- 候场页和 `AnswerComposer` 先读取受保护的 `/interview/speech/status`；缺少识别配置或查询失败时禁用录音入口并允许直接文字回答。状态只表示已配置，不保证供应商实时可用；实际识别失败仍保留录音供用户重试。
