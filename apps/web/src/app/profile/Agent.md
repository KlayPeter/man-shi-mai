...
`RedeemServiceModal` 在请求前按账户 ID 将 `{requestId, packageType}` 存到 localStorage。网络失败、关闭与刷新后只重试同一次兑换；收到服务端同 ID、同类型及余额/次数确认后才清除。存在待确认请求时锁定服务类型，即使本地余额变为 0 也允许重试。后端单次兑换同时扣 20 币并加 1 次权益，流水可按操作 ID 补写；旧客户端缺少 requestId 会被拒绝。

兑换结果以服务端回执更新账户，页面随后重取用户信息与交易记录。`e2e/exchange-local.spec.ts` 在专用数据库合成账户中模拟服务端已兑换而浏览器丢失响应、刷新后重试，核对余额、次数与唯一流水；不涉及真实支付。
