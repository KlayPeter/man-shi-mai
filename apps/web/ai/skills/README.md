...
本目录 (`apps/web/ai/skills/`) 统一管理前端项目中所有供 AI 智能体（或开发人员）遵循的专属规范与技能指南。每个技能作为独立文件夹进行管理，AI 在执行特定操作时应优先调取并遵循对应规范。
...
| :--- | :--- | :--- | :--- | :--- | :--- |
| [automated-testing/](automated-testing/SKILL.md) | automated-testing | `automated-testing` | `1.0.0` | 🟢 开启中 (Active) | guidelines for running automated test suites and build checks. Enforces progressive testing under the test/ directory (e.g., pnpm test test/store/userStore.spec.ts) and Next.js compile verification before making commits. |
