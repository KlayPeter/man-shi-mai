...
- **严格遵守项目规范**：
  - 前端项目：TypeScript + Next.js (App Router) + Zustand 风格。
  - 禁止引入未经许可的第三方包。
...
- **必须使用 Git Commit 技能**：每次提交修改前，必须读取并遵循项目下的 Git 提交规范技能：
  - 本地路径：`ai/skills/git-commit/SKILL.md`
...
2. **测试与编译验证**：修改完成后，若修改涉及状态管理或公共逻辑，必须定位到 `test/` 文件夹下运行对应功能的单元测试（例如 `pnpm test test/store/userStore.spec.ts`）并确保通过。同时必须执行 `pnpm run lint` 和 `pnpm run build`，确保修改没有破坏类型系统、ESLint 校验与构建打包流程。
3. **验证通过放行**：只有在对应的单元测试通过，且编译与语法校验完全通过无误后，才允许提交代码并执行 `git commit`。
